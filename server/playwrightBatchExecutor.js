...
export const executeBatchPlanWithPlaywright = async (
  ...
  try {
    ...
    await page.exposeFunction("__XYZW_AUTOMATION_LOG_BRIDGE__", (entry) => {
      ...
      const entryTokenName = entry?.tokenName || entry?.tokenId;
      
      if (entryTokenName) {
        // If the message doesn't already start with the account name, prepend it.
        if (!message.startsWith(`[${entryTokenName}]`)) {
          message = `[${entryTokenName}] ${message}`;
        }
      } else {
        // Fallback for logs without explicit token context
